...
int run_vm(struct vm *vm1, struct vm *vm2, struct vcpu *vcpu1, struct vcpu *vcpu2, size_t sz)
{
    ...
    {
        sleep(1);
        status = ioctl(vcpu1->vcpu_fd, KVM_RUN, 0);
        if (status < 0)
        {
            perror("KVM_RUN vm1");
            goto check;
        }
        if (vcpu1->kvm_run->exit_reason != KVM_EXIT_IO ||
            vcpu1->kvm_run->io.direction != KVM_EXIT_IO_OUT ||
            vcpu1->kvm_run->io.port != 0xE7)
        {
            fprintf(stderr, "Unexpected exit from vm1\n");
            goto check;
        }
        {
            uint32_t prod_addr;
            char *data = (char *)vcpu1->kvm_run + vcpu1->kvm_run->io.data_offset;
            memcpy(&prod_addr, data, sizeof(prod_addr));

            memcpy(arr, &vm1->mem[prod_addr], sizeof(arr));
        ...
        status = ioctl(vcpu2->vcpu_fd, KVM_RUN, 0);
        if (status < 0)
        {
            perror("KVM_RUN vm2 (phase1)");
            goto check;
        }
        if (vcpu2->kvm_run->exit_reason != KVM_EXIT_IO ||
            vcpu2->kvm_run->io.direction != KVM_EXIT_IO_OUT ||
            vcpu2->kvm_run->io.port != 0xE9)
        {
            fprintf(stderr, "Unexpected exit from vm2 (phase1)\n");
            goto check;
        }
        {
            uint32_t consumer_buf_addr;
            char *data = (char *)vcpu2->kvm_run + vcpu2->kvm_run->io.data_offset;
            memcpy(&consumer_buf_addr, data, sizeof(consumer_buf_addr));

            memcpy(&vm2->mem[consumer_buf_addr], arr, sizeof(arr));
        }

        status = ioctl(vcpu2->vcpu_fd, KVM_RUN, 0);
        if (status < 0)
        {
            perror("KVM_RUN vm2 (phase2)");
            goto check;
        }
        if (vcpu2->kvm_run->exit_reason != KVM_EXIT_IO ||
            vcpu2->kvm_run->io.direction != KVM_EXIT_IO_OUT ||
            vcpu2->kvm_run->io.port != 0xE9)
        {
            fprintf(stderr, "Unexpected exit from vm2 (phase2)\n");
            goto check;
        }
        {
            uint32_t cons_addr;
            uint32_t cons_arr[5];
            char *data = (char *)vcpu2->kvm_run + vcpu2->kvm_run->io.data_offset;
            memcpy(&cons_addr, data, sizeof(cons_addr));

            memcpy(cons_arr, &vm2->mem[cons_addr], sizeof(cons_arr));
