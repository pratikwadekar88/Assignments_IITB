...
struct vcpu
{
	...
	uint32_t num_exit_cnt_add; // Storing the address of variable in guest which will store num_exit cnt
};
		...
		case KVM_EXIT_IO:
		{
			...
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN)
			{
				...
				if (port == 0xE9)
				{
					...
					memcpy((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset, &val, sizeof(val));
					continue;
				}
				else if (port == 0xE6)
				{
