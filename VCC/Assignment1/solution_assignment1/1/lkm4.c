 ...
 *   MODULE_LICENSE: Specifies the licensing; "GPL" ensures compatibility with
 * many kernel symbols.
 */
MODULE_DESCRIPTION("LKM4 - Module to calculate virtual and physical memory "
                   "sizes for a given process.");
MODULE_AUTHOR("CS695");
MODULE_LICENSE("GPL");
 ...
 *
 * Purpose:
 *   Specifies the Process ID (PID) of the process whose memory allocation
 * details will be examined. By default, it is set to 1 (commonly the init
 * process).
 *
 * Details:
 ...
 *
 * Alternative:
 *   Use module_param_named() if you need the external parameter name to differ
 * from the internal variable name.
 */
static int pid = 1;
 ...
 *
 * Purpose:
 *   Calculates the total size (in bytes) of memory pages that are mapped (i.e.,
 * present) within a given Virtual Memory Area (VMA) of a process.
 *
 * Parameters:
 *   - mm: Pointer to the process's memory descriptor (mm_struct), which
 * contains all VMAs.
 *   - vma: Pointer to a specific Virtual Memory Area (vm_area_struct) within
 * the process.
 *
 * Process:
 *   The function iterates from the start (vma->vm_start) to the end
 * (vma->vm_end) of the VMA, stepping through one page (PAGE_SIZE) at a time.
 *
 *   For each virtual address:
 *     1. It performs a page table walk (PGD, P4D, PUD, PMD, and finally PTE) to
 * locate the page entry.
 *     2. If at any level the entry is missing or invalid, it skips the current
 * address.
 *     3. If a valid Page Table Entry (PTE) is found and the page is present,
 * the function increments the 'size' by PAGE_SIZE.
 *
 * Returns:
 *   The total mapped size (in bytes) for the specified VMA.
 *
 * Alternative:
 *   Depending on the architecture or needs, you might consider using existing
 * kernel functions that sum up mapped memory, but this explicit page table walk
 * provides precise control and insight.
 */
unsigned long get_mapped_size(struct mm_struct *mm,
  ...
  for (unsigned long vaddr = vma->vm_start; vaddr < vma->vm_end;
    ...
    if (vaddr & (PAGE_SIZE - 1))
      pr_info(
          "[LKM4] Error in iteration: vaddr 0x%lx not aligned to PAGE_SIZE\n",
          vaddr);
    ...
    // pte_offset_kernel is used for kernel address space; for user space
    // mappings, consider pte_offset_map.
    pte = pte_offset_kernel(pmd, vaddr);
    if (pte) {
      // Unmap the PTE mapping if necessary (this ensures consistency in certain
      // architectures)
      pte_unmap(*pte);

      // If the PTE indicates the page is present, add PAGE_SIZE to the
      // accumulated size.
      if (pte_present(*pte))
        size += PAGE_SIZE;
    }
 ...
 *     4. Estimates the physical memory size using get_mm_rss (a fast but less
 * precise measure).
 *     5. Computes a precise physical memory size by iterating through each VMA
 * and summing the sizes of mapped pages.
 ...
 *
 * Alternative:
 *   - The virtual memory size can be computed iteratively by summing each VMA's
 * range.
 *   - The physical memory size can be determined using different counters in
 * mm->rss_stat, though get_mm_rss() encapsulates these counts.
 */
static int __init print_alloc_size(void) {
  ...
  if (mm != NULL) {
     ...
     *
     * Alternative:
     *   The virtual memory size could also be computed by iterating over each
     * VMA and summing (vma->vm_end - vma->vm_start) for all VMAs.
     */
     ...
     *   - get_mm_rss(mm) returns the resident set size (number of resident
     * pages).
     *   - Converting this to bytes and then to KiB.
     *
     * Alternative:
     *   One can also sum up the various page counts (file-backed, anonymous,
     * shared memory) available in mm->rss_stat. This is essentially what
     * get_mm_rss() does internally.
     */
    physical_size = (get_mm_rss(mm) << PAGE_SHIFT) / 1024;
    pr_info("[LKM4] Estimated Physical Memory Size: %lu KiB\n", physical_size);

    /*
     * Calculate precise physical memory size (this method is more accurate but
     * slower):
     *   - Iterate over each VMA in the process's address space.
     *   - For each VMA, use get_mapped_size() to sum the sizes of pages that
     * are actually mapped.
     *   - Reset physical_size to zero before performing the precise
     * calculation.
     *
     *   The VMA_ITERATOR macro along with for_each_vma simplifies iterating
     * over all VMAs.
     */
    {
      ...
      for_each_vma(iter, vma) { physical_size += get_mapped_size(mm, vma); }
    }
 ...
 *   - module_init() registers the initialization function (print_alloc_size) to
 * be called when the module is loaded.
 *   - module_exit() registers the cleanup function (exit_module) to be called
